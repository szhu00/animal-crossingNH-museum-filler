...
export default function MyCollection(props) {
    ...
    console.log(answer);
    useEffect(() => {
        getJokes(setJokes,setAnswer);
        getCompliment(setCompliment);
     }, [points]);
        ...
        <div>
            {points ? (
                <>
                    ...
                    <Points  setPoints={setPoints} setactive = {props.setactive} display = {jokeDisplay} setDisplay={setJokeDisplay} joke= {joke}
                    setJokes={setJokes} answer= {answer} setAnswer={setAnswer} comDisplay={comDisplay} compliment={Compliment} setComDisplay={setComDisplay}/>
                </>
            )
                : (
                    <>
                        <nav className="navbar">
                            <ul className="navUL">
                                ...
                                <li className="navLI"><button className="navBtn" onClick={() => setPoints(true)}>Other Points</button></li>
...
const getJokes = async (setJokes,setAnswer)=> {
    const j = await jokes.get("random_joke").catch((err) => console.log("err ",err));
    if(j &&j.data) {
        setJokes(j.data.setup);
        setAnswer(j.data.punchline);
    }

}

const getCompliment = async (setCompliment)=> {
    const c = await Compliment.get("api").catch((err) => console.log("err ",err));
    if(c &&c.data) {
        setCompliment(c.data.compliment);
    }
